...
def close_cmd(
    ...
    if not mission_slug:
        console.print("[red]Error:[/red] No mission specified and no active mission detected. Use [cyan]--mission <slug>[/cyan].")
        raise typer.Exit(1)
...
def _teardown_coordination_worktree(
    ...
    *,
    ...
    if CoordinationWorkspace.is_present(repo_root, mission_slug, mid8_value):
        ...
        console.print(f"[green]✓[/green] Coordination worktree torn down for {mission_slug}-{mid8_value}")
...
def follow_up_cmd(
    ...
    if (commit is None) == (pr is None):
        console.print("[red]Error:[/red] supply exactly one of [cyan]--commit <40-hex>[/cyan] or [cyan]--pr <int>[/cyan].")
        raise typer.Exit(1)

    if commit is not None and not re.fullmatch(r"[0-9a-fA-F]{40}", commit):
        console.print(f"[red]Error:[/red] --commit must be a 40-character hex SHA, got {commit!r}.")
        raise typer.Exit(1)
