...
def assert_safe_path_segment(value: str) -> str:
    ...
    if not stripped:
        raise ValueError(f"Not a safe path segment: {value!r} — value must not be empty or whitespace-only.")
    ...
    if value != stripped:
        raise ValueError(f"Not a safe path segment: {value!r} — value must not contain leading or trailing whitespace.")

    # Reject any ".." substring (covers ..foo, foo.., a..b, and literal ..)
    if ".." in stripped:
        raise ValueError(f"Not a safe path segment: {value!r} — value must not contain '..' (traversal guard).")

    # Reject leading dot (covers .hidden, .dot-only, etc.)
    if stripped.startswith("."):
        raise ValueError(f"Not a safe path segment: {value!r} — value must not begin with '.' (traversal guard).")

    # Reject path separators (/ and \) — catches a/b, a\b, /absolute, trailing/
    if "/" in stripped or "\\" in stripped:
        raise ValueError(f"Not a safe path segment: {value!r} — value must not contain path separators.")
