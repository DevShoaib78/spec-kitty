...
def _resolve_take_theirs(
    ...
    if theirs is None:
        ...
        if not ok:
            return None, (f"{RULE_ID_COORDINATION_ARTIFACT}: git rm --sparse {rel_path} failed: {message}")
    ...
    ok, message = _stage_sparse(worktree, rel_path)
    if not ok:
        return None, (f"{RULE_ID_COORDINATION_ARTIFACT}: git add --sparse {rel_path} failed: {message}")
    return _managed_classification(file_path, RULE_ID_COORDINATION_ARTIFACT), None
